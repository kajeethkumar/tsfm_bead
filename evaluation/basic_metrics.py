...
class basic_metricor():
    ...
    def range_recall_new(self, labels, preds, alpha):
        ...
        if len(labels) != len(preds):
            print("Length mismatch", len(labels), len(preds))
            return 0, 0, 0
        ...
        for r in range_label:
            try:
                OverlapReward += (
                    self.w(r, p)
                    * self.Cardinality_factor(r, range_pred)
                )
            except Exception as e:
                print("range error:", e)
                print("r =", r)
                print("range_pred =", range_pred[:10])
                return 0,0,0
    ...
    def range_convers_new(self, label):
        label = np.asarray(label, dtype=np.int64).flatten()

        if len(label) == 0:
            return []

        padded = np.concatenate(([0], label, [0]))
        diff = np.diff(padded)
    ...
    def metric_VUS_pred(self, labels, preds, windowSize):
        ...
        for window in window_3d:
            ...
            P_new = (P + N_labels) / 2
            recall = min(TP / P_new, 1)
            Precision = TP / (N_pred + self.eps)

            recall_3d[window] = recall
            prec_3d[window] = Precision
            f_3d[window] = 2 * Precision * recall / (Precision + recall + self.eps) if (Precision + recall) > 0 else 0
