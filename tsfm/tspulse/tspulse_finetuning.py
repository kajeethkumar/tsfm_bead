...
class TSPulseFineTuner:
    def __init__(
        ...
        # Cache the pristine, pre-trained weights in RAM (CPU) to prevent GPU segfaults
    ...
    def fit(self, X_train, X_val):
        ...
        try:
            ...
            max_finetune_samples = 4224  # 6 months of hourly data, which is a reasonable upper bound for fine-tuning without overfitting
...
if __name__ == "__main__":
    ...
    for b_id in tqdm(df.building_id.unique()[:1], desc="Evaluating buildings"):
       
        X, y = preprocess(df, b_id)
        
        # We explicitly unpack the validation split generated by your preprocessing function
        ...
        # ---------------------------------------------------------
        # 1. Restore to pre-trained base
        detector.reset_weights() 
        
        # 2. Train uniquely on this building's data using true splits
