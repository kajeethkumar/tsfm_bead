...
if __name__ == '__main__':
    ...
    parser.add_argument('--building_id', type=str, default='Unknown', help='Building ID for logging purposes')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            # --- UPDATED: Handle Anomaly Detection Return Values ---
            if args.task_name == 'anomaly_detection':
                f_score, row_data = exp.test(setting)
                
                # Save the results to CSV
                os.makedirs('results', exist_ok=True)
                csv_summary = f"results/{args.model}.csv"
                ...
                print(f"Results appended to {csv_summary} | F-score: {f_score:.4f}")
            else:
        ...
        # --- UPDATED: Handle Anomaly Detection Return Values ---
        if args.task_name == 'anomaly_detection':
            f_score, row_data = exp.test(setting, test=1)
