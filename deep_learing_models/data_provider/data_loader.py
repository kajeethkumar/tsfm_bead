...
class LEADloader(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag='train', 
                 data_path='train.csv', target='meter_reading', scale=True):
        
        self.args = args
        self.root_path = root_path
        self.data_path = data_path
        self.win_size = win_size  # seq_len mapped to win_size
        self.step = step
        self.flag = flag
        self.target = target
        self.scale = scale
    ...
    def __read_data__(self):
        ...
        # 1. Split Logic (244/122 day split)
        num_train_total = 244 * 24
        num_val = 61 * 24
        num_train = num_train_total - num_val
        
        # Separate Features and Labels
        cols_data = [c for c in df_raw.columns if c not in ['timestamp', 'building_id', 'anomaly']]
        df_data = df_raw[cols_data]
        df_labels = df_raw[['anomaly']]

        # 2. Scaling (Fit ONLY on training data to prevent data leakage)
        if self.scale:
        ...
        # 3. Assign sets based on flag
        labels = df_labels.values
        # NEW: Keep track of timestamps
        timestamps = df_raw['timestamp'].values 
        
        if self.flag == 'train':
            self.data = data[:num_train]
            self.labels = labels[:num_train]
            self.timestamps = timestamps[:num_train] # Added
        elif self.flag == 'val':
            self.data = data[num_train:num_train_total]
            self.labels = labels[num_train:num_train_total]
            self.timestamps = timestamps[num_train:num_train_total] # Added
        else: # test
            self.data = data[num_train_total:]
            self.labels = labels[num_train_total:]
            self.timestamps = timestamps[num_train_total:] # Added
...
# #         self.root_path = root_path
# #         self.data_path = data_path
# #         self.win_size = win_size  # Renamed from seq_len to match the factory call
# #         self.step = step
# #         self.flag = flag
# #         self.target = target
# #         self.scale = scale
...
# #         # 1. Split Logic based on your 244/122 day split
# #         # Total = 8784. Train Block = 5856. Test Block = 2928.
# #         num_train_total = 244 * 24
        
# #         # Validation is the last 2 months of the training block (approx 61 days)
# #         num_val = 61 * 24
# #         num_train = num_train_total - num_val
        
# #         # Separate Features and Labels
# #         # We assume 'anomaly' is your ground truth column
# #         cols_data = [c for c in df_raw.columns if c not in ['timestamp', 'building_id', 'anomaly']]
# #         df_data = df_raw[cols_data]
# #         df_labels = df_raw[['anomaly']]
...
# #         # print(f"{self.flag} set: data shape {self.data.shape}, labels shape {self.labels.shape}")

# #     def __len__(self):
# #         # Sliding window length calculation
# #         return (self.data.shape[0] - self.win_size) // self.step + 1
...
# #         seq_x = self.data[s_begin:s_end]
# #         seq_y = self.labels[s_begin:s_end] # Ground truth labels for the window
