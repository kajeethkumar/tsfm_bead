...
class KMeansAD(BaseEstimator, OutlierMixin):
    ...
    def __init__(self, args=None, k=5, window_size=10, stride=1, n_jobs=1, normalize=True):
        """
        Updated __init__ to accept a framework 'args' namespace object dynamically, 
        with fallback defaults for safety.
        """
        super(KMeansAD, self).__init__()
        
        # If framework passes an args object, extract variables dynamically
        if args is not None:
            # Safely get properties from args, falling back to defaults if missing
            self.k = getattr(args, 'k', k)
            self.window_size = getattr(args, 'window_size', window_size)
            self.stride = getattr(args, 'stride', stride)
            self.normalize = getattr(args, 'normalize', normalize)
        else:
            # Fallback to direct positional/keyword arguments
            self.k = k
            self.window_size = window_size
            self.stride = stride
            self.normalize = normalize
