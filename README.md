...
```bash
cd statistical
python mz_score.py --input_csv "Dataset_Path"
python iqr.py --input_csv "Dataset_Path"
```
...
```bash
cd ml_models
python lof.py --input_csv "Dataset_Path"
python iforest.py --input_csv "Dataset_Path"
python k_means.py --input_csv "Dataset_Path"
```
...
To run Crossformer, KANAD, Informer, iTransformer and Non-Stationary Transformer (NST) sequentially, use the provided shell scripts:
 
```bash
bash run_all_1.sh
```
 
You can also use `run_exp.ipynb` for interactive experimentation.
 
Key components:
- `data_provider/` — dataset loading utilities for the LEAD dataset
- `exp/exp_anomaly_detection.py` — training/evaluation experiment driver
- `models/` — model architectures (CNN, LSTM, Crossformer, KANAD, Informer, iTransformer, Non-Stationary Transformer)
- `utils/ad_metrics.py`, `utils/metrics.py` — anomaly detection evaluation metrics
...
```bash
cd tsfm/moment
python moment_zeroshot.py        # Zero-shot evaluation
python moment_finetuning.py       # Fine-tuning on LEAD dataset
```
...
```bash
cd tsfm/units
python units_zeroshot.py          # Zero-shot evaluation
python units_finetuning.py        # Fine-tuning on LEAD dataset
```
...
```bash
cd tsfm/tspulse
python tspulse_zeroshot.py            # Zero-shot evaluation
python tspulse_finetuning_hp.py        # Fine-tuning (with hyperparameter settings) on LEAD dataset
```
